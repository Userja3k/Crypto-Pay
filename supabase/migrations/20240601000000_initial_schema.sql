...
DECLARE v_sender_account_id UUID; v_sender_balance DECIMAL(15,8); v_fee_usd DECIMAL(15,8); v_total_usd DECIMAL(15,8); v_limit cryptopay.limits%ROWTYPE; v_receiver_user_id UUID; v_receiver_account_id UUID; v_tx_id UUID; v_child_limit cryptopay.child_limits%ROWTYPE;
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM cryptopay.users WHERE id = p_sender_user_id AND user_role = 'child') THEN
        ...
        END IF;
        IF v_child_limit.requires_approval AND NOT p_requires_approval THEN
            INSERT INTO cryptopay.transactions (account_id, transaction_type, amount_usd, fee_usd, note, status)
            VALUES (v_sender_account_id, 'send', p_amount_usd, v_fee_usd, p_note, 'pending') RETURNING id INTO v_tx_id;
            ...
            RETURN QUERY SELECT v_tx_id, 'pending_approval'::VARCHAR, NULL::TEXT, 'En attente d''approbation parentale'::TEXT; RETURN;
        END IF;
    END IF;
    ...
    IF p_destination_type = 'internal' THEN
        ...
        UPDATE cryptopay.accounts SET balance_usd = balance_usd - v_total_usd WHERE id = v_sender_account_id;
        UPDATE cryptopay.accounts SET balance_usd = balance_usd + p_amount_usd WHERE id = v_receiver_account_id;
...
RETURNS BOOLEAN AS $$
DECLARE v_app cryptopay.pending_approvals%ROWTYPE;
BEGIN
    ...
    IF p_approve THEN
        ...
        -- On devrait ici déclencher la transaction réelle
    ELSE
